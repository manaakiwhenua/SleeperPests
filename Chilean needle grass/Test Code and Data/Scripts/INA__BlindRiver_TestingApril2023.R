...
###3) Local extinction probability, provided as single number, vector (nodes) or matrix (nodes x timesteps)
##################################################
...
EradicationProb = AnnualEradicationProb*CompleteDetectionProb 
...
###local survival. Try 5% annual decrease
###Would be possible to explore extreme timesteps where 
###conditions drive particularly high or low local survival
#######################################################
SurvivalMatrix = matrix(nrow = nrow(adj), ncol = Ntimesteps)
AnnulDecrease = 0.001
SurvivalMatrix[,1] = runif(nrow(adj),0.9,1)
for(timestep in 2:Ntimesteps)
{
SurvivalMatrix[,timestep] = SurvivalMatrix[,(timestep-1)]*(1-AnnulDecrease*(timestep-1))
}
#plot(1:Ntimesteps,colMeans(SurvivalMatrix)) 
...
ModelName = paste0("Standard","DetProb_",DetectionProb,"_")
...
ManageSD = NULL,
EradicationProb = AnnualEradicationProb, #Annual probability of eradication (must be between 0 and 1)
EradicationSD = NULL,
...
###Declare name for storing results
ModelName = paste0("ManSD_User","DetProb_",DetectionProb,"_")
OutputDir = paste0(RegionResultsDir,ModelName,"/")
dir.create(OutputDir,showWarnings = F)
INApest(
ModelName = ModelName,      #Name for storing results to disk
...
EradicationProb = AnnualEradicationProb, #Annual probability of eradication (must be between 0 and 1)
EradicationSD = runif(1, 0.01,0.05),
SpreadReduction = SpreadReduction,        #Reduction in dispersal probability from nodes adopting management (must be between 0 and 1)
...
ModelName = paste0("ProbEstMatrix","DetProb_",DetectionProb,"_")
OutputDir = paste0(RegionResultsDir,ModelName,"/")
dir.create(OutputDir,showWarnings = F)
INApest(
ModelName = ModelName,      #Name for storing results to disk
...
ModelName = paste0("LocalSurvivalFixed","DetProb_",DetectionProb,"_")
OutputDir = paste0(RegionResultsDir,ModelName,"/")
dir.create(OutputDir,showWarnings = F)
INApest(
ModelName = ModelName,      #Name for storing results to disk
...
InitialInvasion = InitBio,        #Nodes infested at start of simulations
EnvEstabProb = prob_est,           #Environmentally determined establishment probability
Survival = SurvivalMatrix[,1],     #Annual local population survival probability. Set to zero for no local extinction. Can be vector (nodes) or matrix (nodes x timesteps)
SDDprob = adj,                   #Biophysical adjaceny matrix - distance-based disperal probability
SEAM = SEAM,                  ##Socioeconomic adjaceny matrix - info transfer probability between farms
LDDprob = LDDprob,         #Long distance dispersal probability matrix 
geocoords = geocoords,              #XY points for INAscene
...
ModelName = paste0("LocalSurvivalDeclining","DetProb_",DetectionProb,"_")
OutputDir = paste0(RegionResultsDir,ModelName,"/")
dir.create(OutputDir,showWarnings = F)
INApest(
ModelName = ModelName,      #Name for storing results to disk
...
InitialInvasion = InitBio,        #Nodes infested at start of simulations
EnvEstabProb = prob_est,           #Environmentally determined establishment probability
Survival = SurvivalMatrix,     #Annual local population extinction probability. Set to zero for no local extinction. Can be vector (nodes) or matrix (nodes x timesteps)
SDDprob = adj,                   #Biophysical adjaceny matrix - distance-based disperal probability
SEAM = SEAM,                  ##Socioeconomic adjaceny matrix - info transfer probability between farms
LDDprob = LDDprob,         #Long distance dispersal probability matrix 
geocoords = geocoords,              #XY points for INAscene
