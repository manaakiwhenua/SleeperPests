...
#############################################################
###Notes for Robbie
###Use lat longs as axis labels for maps
###Title and filename need to include an additional parameter: "Communication probability"
############################################################

#R:\Projects\SL2116_MPI_SleeperPests\Analysis\Chilean needle grass\Updated Analyses 2022\CurrentFutureClimate\HistoricExamplesLDDMatrixInfoSpread_v2\Current\MARL
#R:\Projects\SL2116_MPI_SleeperPests\Analysis\Chilean needle grass\Updated Analyses 2022\CurrentFutureClimate
#INA_HistoricExample_BlindRiverLDDMatrixInfoSpreadv2.R
getwd()
...
fun.map.region <- function( x = r"[N:\Projects\BaseData\NZ\Boundaries\Loc_body\Region\StatsNZ_v20210209\regional-council-2020-generalised.shp]")
{
  ...
  if ( ! file.exists( fn))
  {
    stop( "Cannot locate regional council shape file", fn)       
  }
  return( x)
}
...
#Test Code----
fun.test.infestationHeatMaps <- function(Nyears = 1, region.idx = 10)
{
  ...
  lst.Regions = c("AUCK", "EBOP", "CANT", "OTAG", "GISB", "WAIK", "HBAY","MNWG", "WELL", "MARL", "STHL", "NRTH", "TNKI",
              "TASM", "NELS")
  #region = 8
  RegionResultsDir <- paste0(ResultsDir,lst.Regions[region.idx],"/")
  ...
  data<-read_csv(paste0(ClimateScenarios[cs],"/Inputs/",lst.Regions[region.idx],"_Simple_points_for_INA.csv"))
  ...
  dist_mat = readRDS(paste0(ClimateScenarios[cs],"/Inputs/",lst.Regions[region.idx],"_dist_mat_farm.rds"))
  ...
  #AnnualErradicationProb = 0.05
  DetectionProbs = c(0.0) #,0.1,0.15,0.2,0.25,0.3) 
  ...
  RegionFarms = sf::st_read(paste0(ClimateScenarios[cs],"/Inputs/",lst.Regions[region.idx],"SheepBeefAtRisk",ClimateScenarios[cs], ".wgs84.shp"))
  ...
  for(detprob in 1:length(DetectionProbs))
  {
    ...
    InvasionFileNameStem = paste0(RegionResultsDir, "\\", "DetProb_", DetectionProb,"_EradProb_",round(AnnualErradicationProb,digits =2),
    ...
    #InvasionFileName = paste0(InvasionFileNameStem, "_InfoLargeOut.rds")
    InvasionFileName = paste0(InvasionFileNameStem, "_InvasionProb.rds")
    if ( ! ( file.exists(InvasionFileName)))
    {
      stop( "Cannot find the invasion probability file", "\r\n", InvasionFileName)
    }
    #InvasionFileName = paste0(RegionResultsDir, "\\DetProb_0.1_ErradProb_0.19_SpreadReduction_0.999_InvasionLargeOut.rds")
    #InvasionFileName = paste0(RegionResultsDir, "\\DetProb_0.1_ErradProb_0.19_SpreadReduction_0.999_InfoLargeOut.rds")
    dir.exists(RegionResultsDir)
    file.exists(InvasionFileName)
    
    InvasionProbResults = readRDS(file = InvasionFileName)
    ...
    class(InvasionProbResults)
    

    Nyears <<- Nyears ##Nyears
    ...
    spf <<- 0.75
    Regions <<- fun.map.region()
    region <<- lst.Regions[region.idx] #2023/02/03
    outlines <<-  FALSE
...
{
  #!! test----
  fun.test.infestationHeatMaps(Nyears = 2, region.idx = 5)
  fun.test.infestationHeatMaps(Nyears = 2, region.idx = 10)
}
