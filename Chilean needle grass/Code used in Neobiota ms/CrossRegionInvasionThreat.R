...
SinkFarmInvasionRisk <- matrix(nrow = nrow(CrossRegionDispersal),ncol = Nyears)
...
SinkFarmInvasionRisk[,year] = rowSums(AnnualInvasion)
DispersalEvents[year] = sum(AnnualInvasion[,])
}
Filename = paste0(OutputFileNameStem,ThreatSource,"_",ThreatSink,"FarmRisk.rds")
saveRDS(SinkFarmInvasionRisk,Filename)
...
Title = paste0("Detection Prob. ",DetectionProb," Eradication Prob. ",round(AnnualEradicationProb,digits =3),
		";\nSpread Reduction ",SpreadReduction," Management prob. ",ManageProb,"\n",
