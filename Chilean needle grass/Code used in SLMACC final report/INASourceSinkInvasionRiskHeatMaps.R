...
#############################################################
#############################################################
###Read in info for invasion risk calcs
#############################################################
#############################################################
...
for(detprob in 1:length(DetectionProbs))
{
  ...
  TitleStem = paste0("Detection Prob. ",DetectionProb," Eradication Prob. ",round(AnnualEradicationProb,digits =3),
                     "\nSpread Reduction ",SpreadReduction)
  InvasionFileNameStem = paste0(RegionResultsDir, "\\"
                                 , fun.basename.DES( d = DetectionProb, e = round(AnnualEradicationProb,digits =2), s = SpreadReduction, short = F, norm = T ))
  ...
  ImageFileNameStem <- fun.basename.DES( d = DetectionProb, e = round(AnnualEradicationProb,digits =2), s = SpreadReduction, short = T,norm = F )
