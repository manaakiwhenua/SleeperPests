...
detprob <- 1
################################
###Zero management Survival = 0.1
################################
ModelName = paste0("StandardCurrentClim_ParallelSurvival")
if(DoClimateChange == TRUE)
  ModelName = paste0("StandardFutureClim_Parallel",DetectionProbs[detprob])
...
INApestMetaParallel(
  ...
  Survival = 0.1,           # local population survival probability. Set to 1 for no environmental limitation on survival. Can be single number, vector (nodes) or matrix (nodes x timesteps)
...
################################
###Zero management EnvEstab = 0.1
################################
ModelName = paste0("StandardCurrentClim_ParallelEnvEstab")
if(DoClimateChange == TRUE)
  ModelName = paste0("StandardFutureClim_ParallelEnvEstab")
OutputDir = paste0(ResultsDir,ModelName,"/")
dir.create(OutputDir)
Start <- Sys.time()
INApestMetaParallel(
  ...
  InvasionRisk = PropTotalHumans*AnnualIncursionRate,  #Vector of probabilities of external invasion risk
  #InitialInfo = sample(1:nrow(d),10),
  EnvEstabProb = 0.1,           #Environmentally determined establishment probability. Can be single value, vector (nodes) or matrix (nodes x timesteps)
