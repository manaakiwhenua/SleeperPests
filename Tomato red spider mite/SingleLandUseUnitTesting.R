############################################################
############################################################
####Test serial and parallel functions give similar results
############################################################
############################################################
...
ModelName = "SingleLandUseZeroMan"
...
INApestMeta(
  ...
  DetectionProb = 0,  #Annual per-individual detection Probability or vector of probabilities vector length nrow(SDDprob)of node adopting management upon detection
...
ModelName = "SingleLandUseDet_0.5"
OutputDir = paste0(ResultsDir,ModelName,"/")
dir.create(OutputDir)
Start <- Sys.time()
set.seed(42)
INApestMeta(
  ...
  Nperm = Nperm,                  #Number of permutations per parameter combination
  Ntimesteps = Ntimesteps,                 #Simulation duration
  DetectionProb = 0.5,  #Annual per-individual detection Probability or vector of probabilities vector length nrow(SDDprob)of node adopting management upon detection
  ManageProb = 0.99,             #Annual Probability or vector of probabilities vector length nrow(SDDprob)of node adopting management upon detection
  MortalityProb = 0.99,           #Annual mortality probability under management
  SpreadReduction = 0.9,        #Reduction in dispersal probability when management adopted. Must be between 0 (no spread reduction) and 1 (complete prevention of spread). Can be single value or vector length nrow(SDDprob)
  InitialPopulation = d$N0,        #Population size at start if simulations
...
ModelName = "SingleLandUseZeroManParallel"
OutputDir = paste0(ResultsDir,ModelName,"/")
dir.create(OutputDir)
Start <- Sys.time()
set.seed(42)
...
ModelName = "SingleLandUseDet_0.5_Parallel"
OutputDir = paste0(ResultsDir,ModelName,"/")
dir.create(OutputDir)
Start <- Sys.time()
set.seed(42)
