###############################################################################
### INApestPathogenTransitionMatrix -- stage-structured pathogen support
###
### Carries pathogen state through node x demographic-stage host dynamics. Host
### survival, stage progression and transition movement preserve pathogen state;
### offspring produced by fecundity enter the susceptible state unless another
### mechanism explicitly changes that assumption.
###
### Transmission can combine spatial node contact with demographic-stage mixing,
### after which pathogen compartments are reconciled to the current host counts.
###############################################################################

# Allocate a fixed integer count across pathogen states without replacement.
...
# Route stage-transition counts among spatial nodes.
...
# Reconcile node x stage x pathogen counts to current host abundance.
...
# Create or validate an explicit node x stage x pathogen-state array.
INApestPathogenStageState <- function(N, Pathogen, InitialState = NULL, Ntimesteps = 1L) {
  ...
  # Convenience seeding is node-level even though subsequent pathogen dynamics
  # may use stage-resolved parameters. Resolve only the initial state counts
  # here; the TM-specific step validates stage/node/time dynamic parameter forms.
  ctx <- list(n_nodes=n, Ntimesteps=as.integer(Ntimesteps))
  resolve_initial_count <- function(x, name) {
    z <- Pathogen$Engine$Resolve(x, 1L, ctx, name)
    if (any(!is.finite(z)) || any(z < 0) || any(z != floor(z)))
      stop(name, " must resolve to non-negative whole-number host counts")
    as.integer(z)
  }
  I <- resolve_initial_count(Pathogen$InitialInfected, "InitialInfected")
  E <- resolve_initial_count(Pathogen$InitialExposed, "InitialExposed")
  R <- resolve_initial_count(Pathogen$InitialRecovered, "InitialRecovered")
  if (!("E" %in% Pathogen$States) && any(E != 0L))
    stop("InitialExposed must be zero unless Model = 'SEIR'")
  if (!("R" %in% Pathogen$States) && any(R != 0L))
    stop("InitialRecovered must be zero unless the model contains R")
  if (any(I + E + R > rowSums(N)))
    stop("Initial pathogen-state counts exceed total host abundance")
  tmp <- matrix(0L, nrow=n, ncol=P, dimnames=list(NULL,Pathogen$States))
  tmp[,"I"] <- I
  if ("E" %in% Pathogen$States) tmp[,"E"] <- E
  if ("R" %in% Pathogen$States) tmp[,"R"] <- R
  for (i in seq_len(n)) {
...
# Resolve pathogen parameters across nodes, stages and timesteps.
...
# Resolve the node contact matrix for pathogen transmission.
...
# Apply transmission, progression and competing infectious exits by stage.
.iptm_pathogen_step <- function(state, Pathogen, timestep, Ntimesteps, StageMixing=NULL) {
  ...
  if (!("R" %in% states) && any(wan != 0)) stop("ImmunityLossProb must be zero unless the model contains R")
  inum_raw <- getp("IntroductionNumber")
  if(any(!is.finite(inum_raw)) || any(inum_raw < 0) || any(inum_raw != floor(inum_raw))) stop("IntroductionNumber must resolve to non-negative whole numbers")
  inum <- as.integer(inum_raw); ds <- getp("DensityScale")
  ...
  list(State=state, Deaths=matrix(deaths,nrow=n_nodes,ncol=D,byrow=TRUE), NewInfections=matrix(newinf,nrow=n_nodes,ncol=D,byrow=TRUE), Introduced=matrix(introduced,nrow=n_nodes,ncol=D,byrow=TRUE))
}

# Advance host stage dynamics while carrying pathogen state with hosts.
