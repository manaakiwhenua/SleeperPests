###############################################################################
### INApestMetaTransitionMatrix -- node x life-stage simulation engine
###
### Adds demographic/life-stage structure to abundance-based INApest. Hosts are
### tracked by node and stage; transition matrices control survival, stasis and
### progression, reproductive stages generate propagules, and movement can also
### occur during stage transitions.
###
### Surveillance, information and management remain separate response states.
### Optional pathogen support expands host state to node x stage x pathogen state
### so infection follows demographic survival, progression and movement.
###############################################################################
...
# Optional demographic-stage x pathogen-state results
...
if(!is.null(Pathogen)) {
  ...
  PathogenStageResults <- array(0, dim=c(nrow(SDDprob), Nstages, length(Pathogen$States), Ntimesteps, Nperm),
                                dimnames=list(NULL,NULL,Pathogen$States,NULL,NULL))
...
if(!is.null(Pathogen)) {
  if(!exists("INApestPathogenStageState", mode="function"))
    stop("Source INApestPathogenTransitionMatrix.R before using Pathogen")
...
HaveInfo = InitInfo
if(!is.null(Pathogen) && isTRUE(Pathogen$DetectionTriggersInfo))
  {
  pdet <- Pathogen$Engine$Resolve(Pathogen$DetectionProb, 1L, list(n_nodes=nrow(SDDprob),Ntimesteps=Ntimesteps), "DetectionProb")
  I_node <- apply(PathogenStageState[, , "I", drop=FALSE], 1, sum)
  InitPathogenDetection <- rbinom(nrow(SDDprob),1,1-(1-pdet)^I_node)
...
if(UseInfoPersistence == T)
  ...
  # Keep demographic x pathogen state coherent with ordinary management mortality.
...
if(!is.null(Pathogen)) {
  saveRDS(PathogenStageResults, paste(OutputDir,ModelName,"PathogenStageLargeOut.rds",sep=""))
  saveRDS(PathogenDeathResults, paste(OutputDir,ModelName,"PathogenDeathLargeOut.rds",sep=""))
  saveRDS(NewInfectionResults, paste(OutputDir,ModelName,"NewInfectionLargeOut.rds",sep=""))
  saveRDS(PathogenDetectedResults, paste(OutputDir,ModelName,"PathogenDetectedLargeOut.rds",sep=""))
}
