...
ExternalPathogenStateProb = NULL, #Optional named pathogen-state probabilities for accepted external host immigrants; NULL means all susceptible
InformationAcquisition = NULL, #Optional local evidence source: host, pathogen, or both. NULL preserves legacy DetectionTriggersInfo behaviour
InfoTriggeredDetectionProb = 0, #Additional per-individual detection probability under pre-existing information
InfoTriggeredDetectionSD = NULL, #Optional uncertainty for information-triggered detection
ReturnResults = FALSE #Return in-memory results while retaining legacy file outputs
)
{
...
if(UsePathogen) {
  ...
  PathogenContext <- list(n_nodes = nrow(SDDprob), n_landuses = Nlanduses, Ntimesteps = Ntimesteps)
...
###Information-triggered surveillance uses the same per-individual land-use
###structure as background detection, but is gated by pre-surveillance HaveInfo.
.ValidateInfoTriggeredDetectionLU <- function(x, name) {
  d <- dim(x)
  if(is.null(d)) {
    if(!(length(x) == 1 || length(x) == Nlanduses))
      stop(name, " must be scalar, length Nlanduses, nodes x land uses, or nodes x land uses x Ntimesteps")
  } else if(length(d) == 2) {
    if(!all(d == c(nrow(SDDprob), Nlanduses))) stop(name, " matrix must have dimensions nodes x land uses")
  } else if(length(d) == 3) {
    if(!all(d == c(nrow(SDDprob), Nlanduses, Ntimesteps))) stop(name, " array must have dimensions nodes x land uses x Ntimesteps")
...
.ExpandInfoTriggeredDetectionLU <- function(x, timestep) {
  d <- dim(x)
  if(is.null(d)) {
    if(length(x) == 1) return(matrix(x, nrow=nrow(SDDprob), ncol=Nlanduses))
...
.DrawInfoTriggeredDetectionLU <- function(timestep) {
  mu <- .ExpandInfoTriggeredDetectionLU(InfoTriggeredDetectionProb, timestep)
  sd <- .ExpandInfoTriggeredDetectionLU(InfoTriggeredDetectionSD, timestep)
  z <- matrix(rnorm(length(mu), mean=as.numeric(mu), sd=as.numeric(sd)), nrow=nrow(SDDprob), ncol=Nlanduses)
  z[z < 0] <- 0; z[z > 1] <- 1; z
}
...
###########################################################
  ...
  ###Observation events and information state are node-level even though
  ###population state is node x land use.
  ...
  BackgroundDetectionProbabilityResults = array(0, dim=c(nrow(SDDprob),Nlanduses,Ntimesteps,Nperm))
  InfoTriggeredDetectionProbabilityResults = array(0, dim=c(nrow(SDDprob),Nlanduses,Ntimesteps,Nperm))
  ...
  if(UsePathogen) {
    ...
    PathogenStateResults <- array(0L, dim = c(nrow(SDDprob), Nlanduses, length(PathogenStates), Ntimesteps, Nperm),
                                  dimnames = list(NULL, NULL, PathogenStates, NULL, NULL))
...
for (perm in 1:Nperm) 
  ...
  ###Draw static information-triggered detectability once per realisation.
  ###Time-varying schedules are drawn inside the timestep loop.
...
###Update info vector for any info spread (if SEAM supplied)
 ...
 ###The host invasion draw and existing matrix recycling/capacity behaviour are
 ###left unchanged; pathogen state follows the accepted host increase afterwards.
 ...
 ###Synchronise accepted external host immigrants with pathogen state.
 if(UsePathogen)
   {
   ...
   N[] <- PathogenStep$N
   for(ss in seq_along(PathogenStates))
     PathogenStateResults[,,ss,timestep,perm] <- matrix(PathogenStateFlat[,ss], nrow = nrow(SDDprob), ncol = Nlanduses)
 ...
 if(UseInfoTriggeredSurveillance)
   ...
   InfoTriggeredDetection = rbinom(1:nrow(SDDprob),size = 1,
     prob = (1-apply(1-LUinfoDetectionProb,1,prod)) * InfoBeforeSurveillance)
   }
 ...
 ###Record host evidence as local evidence only when enabled.
...
if(ReturnResults)
  ...
  class(ResultObject) <- c("INApestMetaMultipleLandUse","list")
