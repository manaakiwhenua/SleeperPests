...
INApestMetaPoint <- function(
  ...
  InfoTriggeredDetectionProb=0,InfoTriggeredDetectionSD=NULL,InfoTriggeredDetectionSpatial=NULL
){
  ...
  if(is.null(ManageSD)&&!is.function(ManageProb))ManageSD<-ManageProb/10;if(is.null(DetectionSD)&&!is.function(DetectionProb))DetectionSD<-DetectionProb/10;if(is.null(InfoTriggeredDetectionSD)&&!is.function(InfoTriggeredDetectionProb))InfoTriggeredDetectionSD<-InfoTriggeredDetectionProb/10;if(is.null(InfoTriggeredDetectionSD)&&is.function(InfoTriggeredDetectionProb))InfoTriggeredDetectionSD<-0;UseInfoTriggeredSurveillance<-is.function(InfoTriggeredDetectionProb)||is.function(InfoTriggeredDetectionSD)||any(as.numeric(InfoTriggeredDetectionProb)!=0,na.rm=TRUE)||any(as.numeric(InfoTriggeredDetectionSD)!=0,na.rm=TRUE);if(is.null(MortalitySD)&&!is.function(MortalityProb))MortalitySD<-MortalityProb/10;if(is.null(FecundityReductionSD))FecundityReductionSD<-FecundityReduction/10;.ipp_validate_probability_schedule(FecundityReductionSD,Ntimesteps,"FecundityReductionSD");if(is.null(SpreadReductionSD)&&!is.function(SpreadReduction))SpreadReductionSD<-(1-SpreadReduction)/10
  ...
  for(perm in seq_len(Nperm)){
    ...
    for(timestep in seq_len(Ntimesteps)){
      if(DoProgress)cat("\r","Realisation",perm,"Timestep",timestep,"...");n_start<-nrow(points);n_natural_deaths<-0L;n_management_deaths<-0L;n_propagules<-0L;n_established<-0L;n_external<-0L;n_new_detections<-0L;n_new_background_detections<-0L;n_new_info_triggered_detections<-0L;n_info_before_surveillance<-0L;n_managing<-0L;n_pathogen_deaths<-0L;n_new_infections<-0L;n_pathogen_detections<-0L;InfoBeforeSurveillancePoint<-logical(0)
      ...
      if(nrow(points)>0L){InfoBeforeSurveillancePoint<-as.logical(points$have_info);n_info_before_surveillance<-sum(InfoBeforeSurveillancePoint);detection_prob<-.ipp_probability_spatial(DetectionProb,DetectionSD,DetectionSpatial,points,timestep,perm,Ntimesteps,"DetectionProb");background_detected_now<-stats::rbinom(nrow(points),1L,detection_prob)==1L;info_detection_prob<-rep(0,nrow(points));info_triggered_detected_now<-rep(FALSE,nrow(points));if(UseInfoTriggeredSurveillance){info_detection_prob<-.ipp_probability_spatial(InfoTriggeredDetectionProb,InfoTriggeredDetectionSD,InfoTriggeredDetectionSpatial,points,timestep,perm,Ntimesteps,"InfoTriggeredDetectionProb");info_triggered_detected_now<-stats::rbinom(nrow(points),1L,info_detection_prob*as.numeric(InfoBeforeSurveillancePoint))==1L};detected_now<-background_detected_now|info_triggered_detected_now;new_detection<-detected_now&!points$detected;n_new_background_detections<-sum(background_detected_now);n_new_info_triggered_detections<-sum(info_triggered_detected_now);n_new_detections<-sum(new_detection);if(any(background_detected_now)){event_i<-event_i+1L;events[[event_i]]<-.ipp_event(perm,timestep,"background_detection",points$id[background_detected_now],points$parent_id[background_detected_now],points$x[background_detected_now],points$y[background_detected_now],"background")};if(any(info_triggered_detected_now)){event_i<-event_i+1L;events[[event_i]]<-.ipp_event(perm,timestep,"info_triggered_detection",points$id[info_triggered_detected_now],points$parent_id[info_triggered_detected_now],points$x[info_triggered_detected_now],points$y[info_triggered_detected_now],"info_triggered")};points$detected[detected_now]<-TRUE;points$have_info[detected_now]<-TRUE;points$last_known_timestep[detected_now]<-timestep;if(any(detected_now))info_sites<-.ipp_add_or_refresh_info_sites(info_sites,points[detected_now,c("id","x","y"),drop=FALSE],timestep);if(any(new_detection)){event_i<-event_i+1L;events[[event_i]]<-.ipp_event(perm,timestep,"detection",points$id[new_detection],points$parent_id[new_detection],points$x[new_detection],points$y[new_detection],"detected")}}
      if(nrow(points)>0L){snapshot_i<-snapshot_i+1L;snapshot<-points;snapshot$have_info_before_surveillance<-InfoBeforeSurveillancePoint;snapshot$background_detection_prob<-detection_prob;snapshot$info_triggered_detection_prob<-info_detection_prob;snapshot$perm<-perm;snapshot$timestep<-timestep;snapcols<-c("perm","timestep","id","parent_id","x","y","stage","birth_timestep","have_info_before_surveillance","background_detection_prob","info_triggered_detection_prob","have_info","detected","managing","last_known_timestep");if(!is.null(Pathogen))snapcols<-c(snapcols,Pathogen$PathogenStateField);snapshots[[snapshot_i]]<-snapshot[,snapcols,drop=FALSE]}
      summary_i<-summary_i+1L;summaries[[summary_i]]<-data.frame(perm=perm,timestep=timestep,n_start=n_start,n_natural_deaths=n_natural_deaths,n_management_deaths=n_management_deaths,n_propagules=n_propagules,n_established=n_established,n_external=n_external,n_end=nrow(points),n_detected=if(nrow(points)>0L)sum(points$detected)else 0L,n_new_detections=n_new_detections,n_new_background_detections=n_new_background_detections,n_new_info_triggered_detections=n_new_info_triggered_detections,n_info_before_surveillance=n_info_before_surveillance,n_pathogen_detections=n_pathogen_detections,n_new_infections=n_new_infections,n_pathogen_deaths=n_pathogen_deaths,n_infectious=if(!is.null(Pathogen)&&nrow(points)>0L)sum(points[[Pathogen$PathogenStateField]]=="I")else 0L,n_have_info=if(nrow(points)>0L)sum(points$have_info)else 0L,n_managing=n_managing,stringsAsFactors=FALSE)
    }
