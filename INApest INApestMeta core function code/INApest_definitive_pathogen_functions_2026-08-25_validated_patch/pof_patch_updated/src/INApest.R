...
INApest <- function(
  ...
  for (perm in seq_len(Nperm)) {
    ...
    InitInfo[InitInfo == 0L] <- InitDetection[InitInfo == 0L]
    ...
    if (UseInfoPersistence) LastKnownPresence[HaveInfo == 1L & Invaded == 1L] <- 0
    ...
    for (timestep in seq_len(Ntimesteps)) {
      ...
      if (UseInfoPersistence) LastKnownPresence[HaveInfo == 1L & Invaded == 1L] <- timestep
      ...
      NewHaveInfo <- stats::rbinom(n, 1L, Invaded * NodeDetectionProb)
      if (UseInfoPersistence) LastKnownPresence[NewHaveInfo == 1L] <- timestep
      HaveInfo[HaveInfo == 0L] <- NewHaveInfo[HaveInfo == 0L]
