###############################################################################
### INApestPointTransitionMatrixParallel -- parallel point-transition wrapper
###
### Runs independent Point Transition Matrix permutations across serial, PSOCK or
### fork workers and combines point histories, event logs and summaries. It uses
### the same explicit-space, stage-transition and optional pathogen processes as
### the serial engine.
###############################################################################

# Create reproducible random-number streams for point-transition workers.
...
# Combine a named data-frame output across parallel permutations.
...
INApestPointTransitionMatrixParallel <- function(
  ModelName = "INApestPointTransitionMatrixParallel", # Model and output name
  ...
  # ---------------------------------------------------------------------------
  # Set up and validate the parallel point-transition run.
  # ---------------------------------------------------------------------------
  ...
  worker <- function(i, model_fun, base_args, rng_stream, model_name) {
    ...
    # Translate worker-local perm=1 back to the global permutation number.
  ...
  # ---------------------------------------------------------------------------
  # Run independent point-transition permutations using the selected backend.
  # ---------------------------------------------------------------------------
