###############################################################################
### INApestMetaPoint -- explicit-point invasion simulation engine
###
### Removes fixed spatial nodes and represents each occurrence as an explicit
### x/y point: for example one individual, nest, colony or other discrete unit.
### New points are generated by dispersal kernels and can respond to fine-scale
### habitat, local capacity and spatially varying surveillance or management.
###
### Information can transfer between nearby points. An optional point pathogen
### interaction attaches disease state directly to each occurrence.
###############################################################################
...
###############################################################################
# Create a native base-R spatial grid used by point models.
...
# Create a habitat grid using the shared INApest spatial-grid contract.
...
# Assign static spatial grids to non-overlapping timestep ranges.
...
# Combine a point probability with its spatial multiplier.
...
###############################################################################
# INApestMetaPoint has a compact frozen signature, so arguments are defined here
# rather than reformatting executable lines:
...
#   Pathogen - Optional point pathogen interaction
#   Survival - Host survival probability between timesteps
#   PropaguleProduction - Propagules produced per host
#   PropaguleEstablishment - Establishment probability for arriving propagules
#   EnvEstabProb - Environmental establishment probability
#   SDDkernel - Short-distance displacement kernel
#   LDDkernel - Long-distance displacement kernel
#   LDDrate - Fraction of propagules entering long-distance dispersal
#   HabitatSuitability - Spatial habitat/establishment suitability surface
#   HabitatSearchRadius - Radius within which new points can seek better habitat
#   HabitatSearchCandidates - Candidate locations sampled during habitat search
#   LocalK - Local weighted carrying-capacity limit
#   KRadius - Radius used to evaluate local carrying capacity
#   DetectionProb - Background-surveillance detection probability
#   DetectionSD - Variation in background detection probability
#   DetectionSpatial - Spatial multiplier for background detection
#   ManageProb - Management probability when information is available
#   ManageSD - Variation in management probability
#   ManageSpatial - Spatial multiplier for management adoption
#   MortalityProb - Management-driven host mortality probability
#   MortalitySD - Variation in management mortality probability
#   MortalitySpatial - Spatial multiplier for management mortality
#   FecundityReduction - Proportional reduction in reproduction under management
#   FecundityReductionSD - Variation in fecundity reduction
#   FecundityReductionSpatial - Spatial multiplier for fecundity reduction
#   SpreadReduction - Proportional reduction in dispersal under management
#   SpreadReductionSD - Variation in spread reduction
#   SpreadReductionSpatial - Spatial multiplier for spread reduction
#   SpreadReductionAppliesTo - Whether spread reduction affects LDD only or all spread
#   InitialInfo - Starting information state
#   InfoRadius - Distance over which information can transfer
#   InfoTransferProb - Probability of information transfer between nearby points
#   InfoKernel - Optional distance kernel for information transfer
#   InfoRetentionProb - Probability existing information persists one timestep
#   InfoPersistenceSteps - Programmed timesteps information persists after evidence
#   ExternalInfoProb - Information arriving from outside the modelled system
#   OngoingExternalInfo - Allow new external information after initialisation
#   ExternalIncursionGenerator - Function generating new external occurrence points
#   OngoingExternalInvasion - Allow new host incursions after initialisation
#   OutputDir - Directory for saved outputs
#   SaveResults - Save standard simulation outputs to disk
#   DoProgress - Print simulation progress to the console
#   Seed - Random seed for reproducible simulations
#   InfoTriggeredDetectionProb - Detection probability where information already exists
#   InfoTriggeredDetectionSD - Variation in information-triggered detection
#   InfoTriggeredDetectionSpatial - Spatial multiplier for information-triggered detection

INApestMetaPoint <- function(
  ModelName="INApestMetaPoint",Nperm,Ntimesteps,InitialPoints=NULL,Pathogen=NULL,
  ...
  # ---------------------------------------------------------------------------
  # Run independent stochastic point histories.
  # ---------------------------------------------------------------------------
  for(perm in seq_len(Nperm)){
    # Resolve uncertain starting locations separately for this simulation.
    # Fixed InitialPoints bypass this block and retain legacy behaviour.
    if(!is.null(InitialPointGenerator)){
      InitialPoints<-InitialPointGenerator(perm=perm)
      if(!is.data.frame(InitialPoints))stop("InitialPointGenerator must return a data.frame.");
      if(!all(c("x","y")%in%names(InitialPoints)))stop("InitialPointGenerator output must contain x and y columns.");
      if(any(!is.finite(InitialPoints$x))||any(!is.finite(InitialPoints$y)))stop("InitialPointGenerator output x and y must be finite.");
    }
    ...
    if(!is.null(InitialInfo)){if(is.logical(InitialInfo)&&length(InitialInfo)==nrow(points))points$have_info<-InitialInfo else if(is.numeric(InitialInfo)){idx<-intersect(as.integer(InitialInfo),seq_len(nrow(points)));points$have_info[idx]<-TRUE}else stop("InitialInfo must be NULL, a logical vector with one value per InitialPoints row, or row indices.")}else if("have_info"%in%names(InitialPoints)){points$have_info<-as.logical(InitialPoints$have_info);points$have_info[is.na(points$have_info)]<-FALSE};if(any(points$have_info))points$last_known_timestep[points$have_info]<-0L
    ...
    # ---------------------------------------------------------------------------
    # Advance point biology, information, response and pathogen state through time.
    # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Combine point histories, event logs and summaries across permutations.
  # ---------------------------------------------------------------------------
