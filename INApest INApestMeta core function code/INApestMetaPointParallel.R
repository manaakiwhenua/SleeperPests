###############################################################################
### INApestMetaPointParallel
###
### Thin parallel wrapper around the serial INApestMetaPoint engine.
### Source INApestMetaPoint.R before calling this function.
###
### Design rule: biological logic lives only in INApestMetaPoint(). Each worker
### executes one serial permutation (Nperm = 1); results are then relabelled
### and combined on the master process.
###############################################################################
...
INApestMetaPointParallel <- function(
  ...
  if (Cores == 1L) {
    ...
    xs <- parallel::mclapply(
      ...
      fun = serial_fun, base_args = args, rng_stream = streams, model_name = ModelName,
