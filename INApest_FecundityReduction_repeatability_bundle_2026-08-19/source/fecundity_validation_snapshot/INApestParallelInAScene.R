...
#######################################################################
###This implements parallel processing with a PSOCK cluster and parallel::parLapply for the permutation loop.
...
UseExternalInfoRefresh = UseInfoPersistence == T || any(InfoRetentionProb < 1)
...
#########################################################
###Use PSOCK parallel processing to increase processing speed
...
Managing = rbinom(1:nrow(SDDprob),size = 1,prob = NodeManageProb*HaveInfo)
  ...
  INA::INAscene(
    ...
    initbio.dist = NA,
    readseam = T,
    seam = if(UseExternalInfoRefresh == T) 0 else SEAM*Detected, ##Only allow information spread from detected extant infestations
    seamdist = NA,
    seamrandp = NA,
    seampla = NA,
    seamplb = NA,
...
###When information loss is active, apply SEAM externally after decay so information
###from other nodes can refresh nodes whose existing information has just expired
if(UseExternalInfoRefresh == T && is.matrix(SEAM) == T)
  {
  RandSEAM = SEAM
...
if(UseExternalInfoRefresh == F)
  {
  ###INAscene returns the existing information state plus newly transferred information,
  ###so isolate nodes that gained information during this timestep before refreshing HaveInfo
  InfoOut = as.vector(LargeOut[[1]]$multout[[1]]$vect1cL[[2]])
  InfoTransferred = ifelse(HaveInfoBeforeSpread == 0 & InfoOut == 1,1,0)
  HaveInfo[HaveInfo == 0] = InfoTransferred[HaveInfo == 0]
  }
...
simplify2array(list(InvasionResultsLoop, ManagingResultsLoop, DetectedResultsLoop), higher = TRUE)
}
