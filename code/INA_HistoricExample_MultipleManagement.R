...
###Vary erradication prob and spread reduction as well as detection probability
##################################################
...
ResultsDir= paste0(main.dir,"/INA_HistoricExample_MultipleManagement/",ClimateScenarios[cs],"/")
...
#      LDDmatrix[,i] = LDDmatrix[,i]/sum(LDDmatrix[,i])*LongDistProbPerFarm
#      }
...
#plot(LDDweight*LongDistProbPerFarm,LDDmatrix)
#abline(0,1)
#hist(LDDmatrix)
#plot(dist_mat,LDDmatrix)
##############################################################
...
ErradicationProbs = c(0,0.05,0.1,0.15,0.2,0.25,0.3,0.5,0.75)
...
DetectionProbs = c(0.05,0.1,0.15,0.25,0.3,0.5,0.75,0.9) 
...
###Choose whether or not to calculate invasion threat to other regions
DoCrossRegion = FALSE

###Loop through different detection probabilities, spread reductions and
###erradication probabilities
for(spreadreduction in 1:length(SpreadReductions))
for(erradprob in 1:length(ErradicationProbs))
...
AnnualErradicationProb = ErradicationProbs[erradprob]
