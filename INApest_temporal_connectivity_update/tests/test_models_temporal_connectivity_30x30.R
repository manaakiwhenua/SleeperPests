...
#
# The tests use a 30 x 30 (900-node) landscape, one realisation and three
...
results["INApest"] <- run_basic_suite(f, "INApest")

f <- load_model("INApestParallel.R", "INApestParallel", c("abind", "doParallel"), parallel = TRUE)
results["INApestParallel"] <- run_basic_suite(f, "INApestParallel")

f <- load_model("INApestParallelInAScene.R", "INApestParallelINAscene", c("abind", "doParallel", "INA"), parallel = TRUE)
results["INApestParallelINAscene"] <- run_basic_suite(f, "INApestParallelINAscene", geocoords)

f <- load_model("INApestMetaParallel.r", "INApestMetaParallel", c("abind", "doParallel"), parallel = TRUE)
results["INApestMetaParallel"] <- run_temporal_suite(f, "INApestMetaParallel", meta_args, "PopulationLargeOut.rds")

f <- load_model("INApestMetaMultipleLandUse.r", "INApestMetaMultipleLandUse")
results["INApestMetaMultipleLandUse"] <- run_temporal_suite(f, "INApestMetaMultipleLandUse", mlu_args, "PopulationLargeOut.rds")

f <- load_model("INApestMetaParallelMultipleLandUse.r", "INApestMetaParallelMultipleLandUse", c("abind", "doParallel"), parallel = TRUE)
results["INApestMetaParallelMultipleLandUse"] <- run_temporal_suite(f, "INApestMetaParallelMultipleLandUse", mlu_args, "PopulationLargeOut.rds")

f <- load_model("INApestMetaTransitionMatrix.r", "INApestMetaTransitionMatrix")
results["INApestMetaTransitionMatrix"] <- run_temporal_suite(f, "INApestMetaTransitionMatrix", transition_args, "PopulationStageLargeOut.rds")

f <- load_model("INApestMetaTransitionMatrixParallel.r", "INApestMetaTransitionMatrixParallel", c("parallel"), parallel = TRUE)
results["INApestMetaTransitionMatrixParallel"] <- run_temporal_suite(f, "INApestMetaTransitionMatrixParallel", transition_args, "PopulationStageLargeOut.rds")

# Guarded-initialisation smoke tests for the two most failure-prone older forms.
# These intentionally provide no initial invasion/population and should now
# complete with a warning rather than error.
message("Running guarded-initialisation smoke tests...")
f <- load_model("INApest.R", "INApest")
if (!is.null(f)) {
  x <- basic_args(zero_route, zero_route)
  x$Ntimesteps <- 1L
  x$InitialInvasion <- NA
  x$InitBioP <- NA
  x$InvasionRisk <- NA
  invisible(run_and_read(f, x, "INApest_no_initial_", "InvasionLargeOut.rds", 5555L))
  message("PASS INApest no-initial-input guard")
}

f <- load_model("INApestMetaMultipleLandUse.r", "INApestMetaMultipleLandUse")
if (!is.null(f)) {
  x <- mlu_args(zero_route, zero_route, 0)
  x$Ntimesteps <- 1L
  x$InitialPopulation <- NA
  x$InitBioP <- NA
  x$InvasionRisk <- NA
  invisible(run_and_read(f, x, "INApestMetaMultipleLandUse_no_initial_", "PopulationLargeOut.rds", 6666L))
  message("PASS INApestMetaMultipleLandUse no-initial-input guard")
}

cat("\nModel-level test summary\n")
for (nm in names(results)) cat(sprintf("%-42s %s\n", nm, if (isTRUE(results[[nm]])) "PASS" else "SKIP"))
if (any(!results)) {
  cat("\nSome optional/package-dependent tests were skipped. Install the named packages and rerun to exercise those functions.\n")
}
cat("\nCompleted 30 x 30 temporal-connectivity model test script.\n")
